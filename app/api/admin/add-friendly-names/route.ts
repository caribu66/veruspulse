...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      await db.query('ALTER TABLE verusid_statistics ADD COLUMN friendly_name VARCHAR(255)');
    ...
    // Verify the updates
    const result = await db.query('SELECT address, friendly_name FROM verusid_statistics ORDER BY network_rank');
