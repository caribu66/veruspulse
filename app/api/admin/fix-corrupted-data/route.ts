...
export async function POST(_request: NextRequest) {
  try {
    ...
    // Reasonable limits for staking rewards
    const MAX_REASONABLE_REWARD_SATS = 100000000000; // 1M VRSC in satoshis
    // const _MAX_REASONABLE_REWARD_VRSC = 1000000; // 1M VRSC - Unused
    ...
    logger.info('📊 Current database state:', {
      ...
      totalAmount: (stats.total_amount / 100000000).toFixed(2) + ' VRSC',
    });
    ...
    // Recalculate verusid_statistics from clean data
    const recalcQuery = `
    ...
    logger.info('📊 Final database state:', {
      ...
      totalAmount: (finalStats.total_amount / 100000000).toFixed(2) + ' VRSC',
    });
    ...
    return NextResponse.json({
      ...
      stats: {
        ...
        totalAmountVRSC: (finalStats.total_amount / 100000000).toFixed(2),
      },
    });
  } catch (error: any) {
    logger.error('❌ Error during data fix:', error);
    return NextResponse.json(
      {
        success: false,
        error: error.message || 'Failed to fix corrupted data',
      },
      { status: 500 }
    );
  }
