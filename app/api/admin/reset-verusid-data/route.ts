...
function generateUTXOStats(totalStakes: number) {
  ...
  const total = Math.floor(totalStakes * baseMultiplier * (Math.random() * 0.5 + 0.5));
  ...
  return {
    ...
    cooldown,
    inactive,
    totalValueSatoshis: Math.floor(total * 1000000000 * (Math.random() * 0.5 + 0.5)),
...
function generateMonthlyData(verusId: any) {
  ...
  for (let i = 11; i >= 0; i--) {
    ...
    data.push({
      ...
      total_rewards_satoshis: Math.floor(Math.max(1000000, rewards * 100000000)),
      apy: Math.max(10, apy),
      efficiency: Math.random() * 0.3 + 0.7
    });
  }
  ...
  try {
    ...
    for (const verusId of knownVerusIDs) {
      ...
      for (const data of monthlyData) {
        ...
        `, [
          verusId.address, 'monthly', data.period_start, data.period_end,
          data.stake_count, data.total_rewards_satoshis, data.apy, data.efficiency
        ]);
      }
      ...
      for (const data of dailyData) {
        ...
        `, [
          verusId.address, 'daily', data.period_start, data.period_end,
          data.stake_count, data.total_rewards_satoshis, data.apy, data.efficiency
        ]);
      }
    }
