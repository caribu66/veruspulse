...
export async function GET(
  ...
  try {
    ...
    console.info({
      ...
      stakingStatsError:
        stakingStatsResponse.status === 'rejected'
          ? stakingStatsResponse.reason
          : stakingStatsResponse.status === 'fulfilled' &&
              !stakingStatsResponse.value.success
            ? stakingStatsResponse.value.error
            : null,
    });
    ...
    if (
      stakingStats?.timeSeries?.daily &&
      Array.isArray(stakingStats.timeSeries.daily)
    ) {
    ...
    const expectedFrequency = calculateExpectedFrequency(
      yourWeight,
      networkWeight
    );
    ...
    const result = {
      ...
      momentum: momentumAnalysis
        ? {
            ...
            // Formatted values
            frequencyTrendFormatted: formatMomentumTrend(
              momentumAnalysis.frequencyTrend as
                | 'increasing'
                | 'stable'
                | 'decreasing'
            ),
            rewardTrendFormatted: formatMomentumTrend(
              momentumAnalysis.rewardTrend as
                | 'increasing'
                | 'stable'
                | 'decreasing'
            ),
