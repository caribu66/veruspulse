...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.info(`🔍 Browsing VerusIDs - search: "${search}", limit: ${limit}`);
    ...
    logger.info(`📊 Executing query with params:`, queryParams);
    ...
    logger.info(`✅ Found ${result.rows.length} identities (total: ${total})`);
    ...
    logger.error('❌ Error browsing VerusIDs:', error);
    return NextResponse.json(
      {
        success: false,
