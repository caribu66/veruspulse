...
import { VerusIDBrowseData, FilterState, SortOptions, ViewMode } from '@/lib/types/verusid-browse-types';
import {
...
export function BrowseAllVerusIDs() {
  ...
  const loadIdentities = async () => {
    try {
      ...
      if (result?.success && result.data) {
        ...
        const mappedIdentities = result.data.identities.map((identity: any) => ({
          ...
          activityStatus: identity.lastStake ? 'active' : (identity.totalValueVRSC > 0 ? 'active' : 'inactive'),
          daysSinceLastStake: identity.lastStake ? 
            Math.floor((Date.now() - new Date(identity.lastStake).getTime()) / (1000 * 60 * 60 * 24)) : null,
        }));
  ...
  const filteredAndSortedIdentities = useMemo(() => {
    ...
    const uniqueIdentities = allIdentities.filter((identity, index, self) => 
    ...
    // Apply sorting
    filtered = sortIdentities(filtered, sortOptions.sortBy, sortOptions.sortOrder);

    return filtered;
  }, [identities, rpcSearchResults, filters, sortOptions]);

  // Pagination
  const totalPages = Math.ceil(filteredAndSortedIdentities.length / ITEMS_PER_PAGE);
  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
  const endIndex = startIndex + ITEMS_PER_PAGE;
  const paginatedIdentities = filteredAndSortedIdentities.slice(startIndex, endIndex);
        ...
        <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-8">
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1">
              <h3 className="text-red-400 font-semibold text-lg mb-2">Error Loading Identities</h3>
      ...
      <div className="bg-slate-900 rounded-2xl p-8 border border-slate-700">
        <div className="flex items-center justify-between mb-6">
          <div>
            <div className="flex items-center space-x-3 mb-2">
              ...
              <h1 className="text-3xl font-bold text-white">Browse All VerusIDs</h1>
            </div>
            <p className="text-blue-200">
              Explore all registered VerusID identities with comprehensive staking analytics
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-white/10 rounded-lg p-4">
            ...
            <div className="text-2xl font-bold text-white">{filteredAndSortedIdentities.length}</div>
          </div>
          <div className="bg-white/10 rounded-lg p-4">
            <div className="flex items-center space-x-2 mb-1">
              <Database className="h-4 w-4 text-purple-300" />
              <span className="text-sm text-blue-200">Page {currentPage} of {totalPages}</span>
            </div>
            <div className="text-2xl font-bold text-white">
              {startIndex + 1}-{Math.min(endIndex, filteredAndSortedIdentities.length)}
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="bg-slate-900 rounded-2xl border border-slate-700 p-6">
          <div className="flex items-center justify-between">
            <div className="text-sm text-blue-200">
              Showing {startIndex + 1} to {Math.min(endIndex, filteredAndSortedIdentities.length)} of{' '}
