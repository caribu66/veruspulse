...
// const VerusRPC = require('verus-rpc'); // Not needed for this scanner

class Early2022GapScanner {
  constructor() {
    this.pool = new Pool({
      connectionString:
        'postgresql://verus_user:verus_secure_2024@localhost:5432/verus_utxo_db',
    });
    // RPC client not needed - using execSync for verus commands
    this.stakesFound = 0;
    this.blocksProcessed = 0;
    this.posBlocksFound = 0;
    this.startTime = Date.now();
    this.rpcDelay = 0; // Minimal delay for speed
    this.batchSize = 2000; // Larger batches for speed
    this.batchDelay = 25; // Minimal delay between batches
    this.retryDelay = 50; // Reasonable retry delay
  }

  async initialize() {
    console.log('🎯 EARLY 2022 GAP SCANNER');
    console.log('==========================');
  ...
  async processBlock(blockHeight) {
    try {
      // Add small delay to prevent ENOBUFS
      if (this.rpcDelay > 0) {
      ...
      if (error.message.includes('ENOBUFS')) {
        console.log(
          `⚠️  ENOBUFS error at block ${blockHeight}, retrying in ${this.retryDelay}ms...`
        );
        await new Promise(resolve => setTimeout(resolve, this.retryDelay));
        return await this.processBlock(blockHeight); // Retry
      }
  ...
  async run() {
    try {
      ...
      const startBlock = 2300000; // Early 2022
      ...
      console.log('');
      console.log('🎉 Early 2022 gap scan completed!');
      console.log(
...
// Run the scanner
const scanner = new Early2022GapScanner();
