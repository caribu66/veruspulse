 ...
 */
export function calculateStakingMomentum(recentStakes: Array<{ timestamp: Date; amount: number }>) {
  const now = new Date();
  const sevenDaysAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
  const thirtyDaysAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);

  // Filter stakes by time periods
  const last7Days = recentStakes.filter(stake => stake.timestamp >= sevenDaysAgo);
  const last30Days = recentStakes.filter(stake => stake.timestamp >= thirtyDaysAgo);
  const previous7Days = recentStakes.filter(
    stake => stake.timestamp >= new Date(thirtyDaysAgo.getTime() + 7 * 24 * 60 * 60 * 1000) 
  ...
  const frequencyTrend = frequency7d > frequencyPrevious7d ? 'increasing' : 
                        frequency7d < frequencyPrevious7d ? 'decreasing' : 'stable';
  
  const frequencyChange = frequencyPrevious7d > 0 ? 
    ((frequency7d - frequencyPrevious7d) / frequencyPrevious7d) * 100 : 0;

  // Calculate reward momentum (if we have reward data)
  const reward7d = last7Days.reduce((sum, stake) => sum + stake.amount, 0);
  const rewardPrevious7d = previous7Days.reduce((sum, stake) => sum + stake.amount, 0);
  
  const rewardTrend = reward7d > rewardPrevious7d ? 'increasing' : 
                     reward7d < rewardPrevious7d ? 'decreasing' : 'stable';
  
  const rewardChange = rewardPrevious7d > 0 ? 
    ((reward7d - rewardPrevious7d) / rewardPrevious7d) * 100 : 0;
  ...
  return {
    ...
    momentumScore: calculateMomentumScore(frequencyTrend, rewardTrend, frequencyChange, rewardChange),
    
    // Activity indicators
    lastStakeDays: recentStakes.length > 0 ? 
      Math.floor((now.getTime() - recentStakes[recentStakes.length - 1].timestamp.getTime()) / (1000 * 60 * 60 * 24)) : null,
 ...
 */
export function formatMomentumTrend(trend: 'increasing' | 'stable' | 'decreasing'): string {
  switch (trend) {
    case 'increasing': return 'Accelerating';
    case 'decreasing': return 'Decelerating';
 ...
 */
export function calculateExpectedFrequency(yourWeight: number, networkWeight: number): number {
 ...
 */
export function calculatePerformanceRatio(actualFrequency: number, expectedFrequency: number): number {
