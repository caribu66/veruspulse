...
export default function () {
  ...
  if (Math.random() < 0.3) {
    makeRequest(`${BASE_URL}/api/verusids/browse?page=1&limit=20`, 'verusids-browse');
    sleep(1);
  ...
  if (Math.random() < 0.2 && sampleIAddresses.length > 0) {
    const iAddr = sampleIAddresses[Math.floor(Math.random() * sampleIAddresses.length)];
    makeRequest(`${BASE_URL}/api/verusid/${iAddr}/stats`, 'verusid-stats');
    sleep(1);
    
    // User checks staking stats for this ID
    makeRequest(`${BASE_URL}/api/verusid/${iAddr}/staking-stats`, 'verusid-staking-stats');
    sleep(1);
  }
  ...
  if (Math.random() < 0.15) {
    makeRequest(`${BASE_URL}/api/verusids/autocomplete?query=ver`, 'verusids-autocomplete');
    sleep(0.5);
  }
...
export function handleSummary(data) {
  ...
  const summary = {
    'Test Summary': {
      ...
      'Failed Requests': (httpReqFailed.values && httpReqFailed.values.passes) || 0,
      ...
      'Requests/sec': ((httpReqs.values && httpReqs.values.rate) || 0).toFixed(2),
    }
  };
