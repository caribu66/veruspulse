...
echo "📊 EARLY 2022 GAP SCANNER MONITOR"
echo "=================================="
echo ""

# Check if scanner is running
if pgrep -f "early-2022-gap-scanner" > /dev/null; then
    echo "✅ Scanner is running (PID: $(pgrep -f 'early-2022-gap-scanner'))"
    echo ""
    
    # Show recent activity
    echo "📈 RECENT ACTIVITY:"
    tail -10 early-2022-gap-scanner.log
    echo ""
    
    # Calculate progress
    echo "📊 PROGRESS CALCULATION:"
    node -e "
    // Calculate progress for Early 2022 scanner (2.3M-2.4M range)
    ...
    try {
        const logContent = fs.readFileSync('early-2022-gap-scanner.log', 'utf8');
    ...
    echo "   Start scanner: nohup node early-2022-gap-scanner.js > early-2022-gap-scanner.log 2>&1 &"
    echo "   View log: tail -f early-2022-gap-scanner.log"
    echo "   Stop scanner: kill \$(pgrep -f 'early-2022-gap-scanner')"
fi

echo ""
echo "🔄 Refresh this monitor: ./monitor-early-2022.sh"
