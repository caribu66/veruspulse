...
echo "⚡ FAST EARLY 2022 SCANNER MONITOR"
echo "=================================="
echo ""

# Check if scanner is running
if pgrep -f "robust-early-2022-scanner" > /dev/null; then
    echo "✅ Fast scanner is running (PID: $(pgrep -f 'robust-early-2022-scanner'))"
    echo ""
    
    # Show recent activity
    echo "📈 RECENT ACTIVITY:"
    tail -10 fast-early-2022-scanner.log
    echo ""
    
    # Calculate progress
    echo "📊 PROGRESS CALCULATION:"
    node -e "
    // Calculate progress for Fast Early 2022 scanner (2.3M-2.4M range)
    ...
    try {
        const logContent = fs.readFileSync('fast-early-2022-scanner.log', 'utf8');
    ...
    // Calculate ETA
    const speed = 25; // blocks/sec from optimized settings
    const etaSeconds = remainingBlocks / speed;
    const etaMinutes = Math.round(etaSeconds / 60);
    const etaHours = Math.round(etaMinutes / 60);
    
    console.log('   ⚡ Speed: ~' + speed + ' blocks/sec (optimized)');
    ...
    echo "❌ Fast scanner is not running"
    echo ""
    echo "📋 Available commands:"
    echo "   Start scanner: nohup node robust-early-2022-scanner.js > fast-early-2022-scanner.log 2>&1 &"
    echo "   View log: tail -f fast-early-2022-scanner.log"
    echo "   Stop scanner: kill \$(pgrep -f 'robust-early-2022-scanner')"
fi

echo ""
echo "🔄 Refresh this monitor: ./monitor-fast-early-2022.sh"
