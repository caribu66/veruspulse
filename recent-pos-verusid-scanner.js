...
class RecentPoSVerusIDScanner {
  ...
  async initialize() {
    console.log('🚀 RECENT PoS VERUSID SCANNER - Focus on Recent Blocks');
    console.log('====================================================');
    console.log('');
    console.log(
      '🎯 MISSION: Find PoS blocks with I-addresses in RECENT blocks'
    );
    console.log(
      '🔍 METHOD: Check recent blocks where VerusID PoS activity is higher'
    );
    console.log(
      '⚡ FOCUS: Recent blocks (3.5M+) where we found PoS VerusID stakes'
    );
    console.log('');
  }
  ...
  async run() {
    try {
      ...
      const startBlock = 3500000; // Recent blocks where we found PoS VerusID activity

      console.log(
        `📊 Current blockchain height: ${currentHeight.toLocaleString()}`
      );
      console.log(`📊 Start block (Recent): ${startBlock.toLocaleString()}`);
      console.log('');

      // Get missing blocks
      console.log('🔍 Finding missing blocks in recent range...');
      const missingBlocks = await this.getMissingBlocks(
        startBlock,
        currentHeight
      );
      console.log(
        `📊 Found ${missingBlocks.length.toLocaleString()} missing blocks in recent range`
      );
      console.log('');

      if (missingBlocks.length === 0) {
        console.log('✅ No missing blocks found in recent range!');
        return;
      }
      ...
      await this.scanMissingBlocks(
        missingBlocks,
        `Recent PoS VerusID stakes scan (${missingBlocks.length.toLocaleString()} blocks)`
      );

      // Final summary
      const totalTime = (Date.now() - this.startTime) / 1000 / 60;
      console.log('╔═══════════════════════════════════════════════╗');
      console.log('║              RECENT PoS VERUSID COMPLETE!   ║');
      console.log('╚═══════════════════════════════════════════════╝');
      console.log(
...
// Run the scanner
const scanner = new RecentPoSVerusIDScanner();
