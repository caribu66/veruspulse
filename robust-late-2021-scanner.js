...
/**
 * Robust Late 2021 Gap Scanner
 * - Targets the major gap in Late 2021 (2.2M-2.3M) where only 4 stakes exist
 * - Uses Oink70's proven PoS detection method
 * - Enhanced with robust RPC rate limiting to prevent ENOBUFS errors
...
class RobustLate2021Scanner {
  constructor() {
    ...
    this.rpcDelay = 10; // Minimal delay for speed
    this.batchSize = 1000; // Larger batches for speed
    this.batchDelay = 100; // Minimal delay between batches
    this.retryDelay = 200; // Reasonable retry delay
  }

  async initialize() {
    console.log('🎯 ROBUST LATE 2021 GAP SCANNER');
    console.log('================================');
    console.log('');
    console.log("🔧 Using Oink70's proven PoS detection method");
    console.log('🎯 Targeting major gap: Late 2021 (2.2M-2.3M)');
    console.log('📊 Current stakes in range: 4 (should be ~50,000)');
    console.log('⚡ Enhanced with robust RPC rate limiting');
    console.log('🛡️  Designed to prevent ENOBUFS errors');
    console.log('');
  }
  ...
  async processBlock(blockHeight) {
    ...
    while (retries > 0) {
      try {
        ...
        if (retries > 0) {
          console.log(
            `⏳ Retrying block ${blockHeight} in ${this.retryDelay}ms...`
          );
  ...
  async run() {
    try {
      ...
      // Target the Late 2021 gap (2.2M-2.3M)
      const gapStart = 2200000; // 2.2M
      const gapEnd = 2300000; // 2.3M

      console.log('');
      console.log('🎯 Late 2021 Gap Range:');
      console.log(`   Start: Block ${gapStart.toLocaleString()} (Late 2021)`);
      console.log(`   End: Block ${gapEnd.toLocaleString()} (Late 2021)`);
      console.log(
      ...
      console.log(`   Target speed: ~30 blocks/sec`);
      console.log('');
      ...
      console.log('\n🎉 Robust Late 2021 gap scan completed!');
...
// Run the scanner
const scanner = new RobustLate2021Scanner();
