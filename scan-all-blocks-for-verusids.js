...
class ScanAllBlocksForVerusIDs {
  constructor() {
    this.pool = new Pool({
      connectionString:
  ...
  async initialize() {
    console.log('🚀 SCAN ALL BLOCKS FOR VERUSIDS - Check PoS AND PoW Blocks');
    console.log('=======================================================');
    console.log('');
    console.log(
      '🎯 MISSION: Check ALL blocks for VerusID addresses (not just PoS)'
    );
    console.log('🔍 METHOD: Look for I-addresses in ALL transaction outputs');
  ...
  async extractVerusIDAddressesFromBlock(block, blockHeight) {
    const verusIDAddresses = [];

    if (!block.tx || block.tx.length === 0) {
      return verusIDAddresses;
    }

    // Check ALL transactions in the block, not just the first one
          ...
          if (address && address.startsWith('i') && address.length > 20) {
            ...
            if (exists) {
              verusIDAddresses.push({
                ...
                classifier: 'verusid_transaction',
                sourceAddress: address,
                blockType: block.validationtype || block.blocktype || 'unknown',
              });
            }
          }
        }
  ...
  async saveVerusIDAddresses(addresses) {
  ...
  async scanMissingBlocks(missingBlocks, description) {
    ...
    for (const blockHeight of missingBlocks) {
      try {
        ...
        const verusIDAddresses = await this.extractVerusIDAddressesFromBlock(
          block,
          blockHeight
        );
        batchAddresses.push(...verusIDAddresses);

        this.blocksProcessed++;
        processed++;

        // Process in batches
        if (batchAddresses.length >= this.batchSize) {
          await this.saveVerusIDAddresses(batchAddresses);
          this.stakesFound += batchAddresses.length;
          batchAddresses = [];
        }
        ...
        if (now - lastProgressTime > 10000) {
          ...
          console.log(
            `📊 Progress: ${progress}% | Blocks: ${processed.toLocaleString()}/${totalBlocks.toLocaleString()} | VerusIDs: ${this.stakesFound.toLocaleString()} | Rate: ${rate.toFixed(1)}/s | ETA: ${eta}m`
          );
    ...
    console.log(
      `📊 Found ${this.stakesFound.toLocaleString()} VerusID addresses so far`
    );
    console.log('');
  }
  ...
  async run() {
    try {
      ...
      await this.scanMissingBlocks(
        missingBlocks,
        `All blocks scan for VerusIDs (${missingBlocks.length.toLocaleString()} blocks)`
      );

      // Final summary
      const totalTime = (Date.now() - this.startTime) / 1000 / 60;
      console.log('╔═══════════════════════════════════════════════╗');
      console.log('║              ALL BLOCKS SCAN COMPLETE!      ║');
      console.log('╚═══════════════════════════════════════════════╝');
      console.log(
        `📊 Blocks processed: ${this.blocksProcessed.toLocaleString()}`
      );
      console.log(
        `🎯 VerusID addresses found: ${this.stakesFound.toLocaleString()}`
      );
...
// Run the scanner
const scanner = new ScanAllBlocksForVerusIDs();
