...
class ScanMissingBlocks {
  ...
  async initialize() {
    console.log('🚀 SCAN MISSING BLOCKS - Only Scan Blocks Without Stakes');
    console.log('=======================================================');
    console.log('');
    console.log('🎯 MISSION: Scan only blocks that have NO stakes recorded');
    console.log('🔍 METHOD: Skip blocks that already have stakes in database');
  ...
  async run() {
    try {
      ...
      await this.scanMissingBlocks(
        missingBlocks,
        `Missing blocks scan (${missingBlocks.length.toLocaleString()} blocks)`
      );

      // Final summary
      const totalTime = (Date.now() - this.startTime) / 1000 / 60;
      console.log('╔═══════════════════════════════════════════════╗');
      console.log('║              MISSING BLOCKS SCAN COMPLETE!  ║');
      console.log('╚═══════════════════════════════════════════════╝');
      console.log(
...
// Run the scanner
const scanner = new ScanMissingBlocks();
