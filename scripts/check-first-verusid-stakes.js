#!/usr/bin/env node
/**
 * check-first-verusid-stakes.js
 * Check the first VerusID (VCF.VRSC@) for early stakes
 */
...
const FIRST_VERUSID_ADDRESS = 'iQUHYRcKtgB7xXV1y5NpvzSK3Qb6ToNAqg'; // VCF.VRSC@

async function rpcCall(method, params = []) {
  const auth = Buffer.from(`${RPC_USER}:${RPC_PASS}`).toString('base64');
  const postData = JSON.stringify({
    jsonrpc: '1.0',
    id: 'check-first-verusid-stakes',
    method,
    params,
  });
...
async function checkFirstVerusIDStakes() {
  console.log('🔍 Checking First VerusID (VCF.VRSC@) for early stakes...\n');
  console.log(`Target address: ${FIRST_VERUSID_ADDRESS}\n`);

  try {
    // Search from VerusID activation to a reasonable range
    const startHeight = 800208; // First VerusID creation
    const endHeight = 2000000; // Go well beyond known stakes
    const step = 5000; // Check every 5000th block for efficiency
    ...
    for (let height = startHeight; height <= endHeight; height += step) {
      try {
        ...
        const stakes = findStakesInBlock(block, FIRST_VERUSID_ADDRESS);
        ...
        if (stakes.length > 0) {
          ...
          stakes.forEach(stake => {
            console.log(
              `🎯 Block ${stake.blockHeight}: VCF.VRSC@ stake found!`
            );
    ...
    console.log(`   VCF.VRSC@ stakes found: ${stakesFound}`);

    if (earliestStake) {
      console.log(`\n🏆 Earliest VCF.VRSC@ Stake:`);
      console.log(
    ...
    if (stakesFound === 0) {
      console.log('\n❌ No VCF.VRSC@ stakes found in this range!');
      console.log(
        '   This confirms that VerusID staking began much later than creation.'
      );
    } else {
      console.log('\n✅ VCF.VRSC@ stakes found!');
      console.log(
        '   This would indicate earlier VerusID staking than expected.'
      );
    }
  } catch (error) {
...
checkFirstVerusIDStakes().catch(console.error);
