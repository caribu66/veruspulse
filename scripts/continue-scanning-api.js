#!/usr/bin/env node
/**
 * continue-scanning-api.js
 * Continues scanning using the existing API infrastructure
 */
...
console.log('╔═══════════════════════════════════════════════╗');
console.log('║   Continue VerusID Scanning Service (API)     ║');
console.log('╚═══════════════════════════════════════════════╝\n');
...
// Get current blockchain height from API
async function getCurrentBlockHeight() {
  try {
    const response = await fetch('http://localhost:3000/api/consolidated-data');
    const data = await response.json();
    return data.data.blockchain.blocks;
  } catch (error) {
...
async function getBlockData(blockHeight) {
  try {
    const response = await fetch(
      `http://localhost:3000/api/block/${blockHeight}`
    );
    if (!response.ok) {
      return null;
    }
...
async function scanBlock(blockHeight, verusIDMap) {
  try {
    const block = await getBlockData(blockHeight);
...
// Main loop
async function main() {
  console.log('🚀 Starting continuous VerusID scanning using API...\n');
