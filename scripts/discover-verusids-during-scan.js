  ...
  while (true) {
    try {
      ...
      const currentTotal = await db.query(
        "SELECT COUNT(*) as count FROM identities WHERE identity_address LIKE 'i%'"
      );
      ...
      const scanUpTo = Math.max(
        stats.lastCheckBlock,
        highestScannedBlock - 1000
      );
      ...
      if (scanUpTo > stats.lastCheckBlock) {
        ...
        console.log(
          `   (Following stake scanner at block ${highestScannedBlock})`
        );
        ...
        console.log(
          `   Found ${addresses.size} I-addresses in ${scanTo - scanFrom + 1} blocks`
        );
