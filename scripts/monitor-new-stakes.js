...
async function getCurrentHeight() {
  return new Promise((resolve, reject) => {
    http.get(`${API_BASE}/api/consolidated-data`, (res) => {
      let data = '';
...
async function getBlock(height) {
  return new Promise((resolve, reject) => {
    http.get(`${API_BASE}/api/latest-blocks?limit=1&offset=0`, (res) => {
      let data = '';
      res.on('data', chunk => data += chunk);
      res.on('end', () => {
        try {
    ...
    for (let height = lastCheckedHeight + 1; height <= currentHeight; height++) {
      ...
      for (const stake of stakes) {
        if (await recordStake(stake)) {
          newStakes++;
          console.log(`   ✨ NEW STAKE! Block ${stake.blockHeight}: ${stake.address} earned ${stake.reward.toFixed(4)} VRSC`);
        }
      }
    }
