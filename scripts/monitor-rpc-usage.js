...
class RPCMonitor {
  ...
  async displayLiveStats() {
    ...
    console.log(
      `Tracking: ${liveStats.totalTrackedRequests} requests in current window\n`
    );
    ...
    this.displayBar(
      'Per Second',
      liveStats.current.perSecond,
      liveStats.limits.perSecond,
      usage.perSecond
    );
    this.displayBar(
      'Per Minute',
      liveStats.current.perMinute,
      liveStats.limits.perMinute,
      usage.perMinute
    );
    this.displayBar(
      'Per Hour',
      liveStats.current.perHour,
      liveStats.limits.perHour,
      usage.perHour
    );
    ...
    console.log('─────────────────────────────────────────────────────');
    console.log(
      `  Per Second:  ${liveStats.available.perSecond} requests available`
    );
    console.log(
      `  Per Minute:  ${liveStats.available.perMinute} requests available`
    );
    console.log(
      `  Per Hour:    ${liveStats.available.perHour} requests available`
    );
    console.log(
      `  Burst Tokens: ${liveStats.available.burst}/${liveStats.limits.burst} tokens`
    );
    ...
    console.log('\n─────────────────────────────────────────────────────');
    console.log(
      `Updates every ${STATS_INTERVAL / 1000}s | Press Ctrl+C to exit\n`
    );
  }
