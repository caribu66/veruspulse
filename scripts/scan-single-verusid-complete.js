...
if (!TARGET_ID) {
  ...
  console.error('Example: node scan-single-verusid-complete.js iPZkWmFAhQSFsgKiLExowaoXvzaor2bBZ5');
  process.exit(1);
}
...
async function scanCompleteHistory(targetAddress, startHeight, endHeight) {
  console.log(`Scanning blocks ${startHeight.toLocaleString()} to ${endHeight.toLocaleString()}...\n`);
    ...
    for (let h = height; h <= batchEnd; h++) {
      try {
        ...
        if (stats.blocksScanned % 1000 === 0) {
          ...
          console.log(`ETA: ${(eta / 60).toFixed(1)} min (${(eta / 3600).toFixed(1)} hours)`);
          console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`);
        }
      } catch (error) {
...
async function main() {
  try {
    ...
    const deleted = await db.query('DELETE FROM staking_rewards WHERE identity_address = $1', [TARGET_ID]);
    ...
    console.log(`   Start: ${VERUSID_ACTIVATION_BLOCK.toLocaleString()} (VerusID activation)`);
    console.log(`   End: ${currentHeight.toLocaleString()}`);
    console.log(`   Total blocks: ${(currentHeight - VERUSID_ACTIVATION_BLOCK).toLocaleString()}\n`);
    
    // Scan
    stats.startTime = Date.now();
    await scanCompleteHistory(TARGET_ID, VERUSID_ACTIVATION_BLOCK, currentHeight);
