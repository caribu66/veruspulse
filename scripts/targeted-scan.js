    ...
    try {
      ...
      for (const block of response.blocks) {
        ...
        for (const stake of stakes) {
          ...
          console.log(`📍 Block ${stake.blockHeight}: ${stake.address} staked ${stake.amount} VRSC`);
        }
      ...
      if (processedBlocks % 100 === 0) {
        ...
        console.log(`Progress: ${processedBlocks} blocks, ${stats.stakeEventsFound} stakes found (${rate.toFixed(1)} blocks/sec)`);
      }
...
Examples:
  ...
  node targeted-scan.js --blocks 3800000
    ...
    const dbTest = await db.query('SELECT COUNT(*) FROM stake_events WHERE address = ANY($1)', [TARGET_ADDRESSES]);
    ...
    console.log(`Rate: ${(stats.blocksScanned / totalTime).toFixed(1)} blocks/sec\n`);
