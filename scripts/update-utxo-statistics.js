...
async function rpcCall(method, params = []) {
  ...
  const rpcHost = '192.168.86.89:18843'; // Use the working host
  ...
  const cmd = `curl -s --user ${rpcUser}:${rpcPass} --data-binary '${escapedData}' -H 'content-type: text/plain;' http://${rpcHost}/`;
...
async function getUTXOData(address, currentHeight) {
  try {
    ...
    for (const utxo of utxos) {
      ...
      if (confirmations >= MIN_CONFIRMATIONS_FOR_STAKING) {
        ...
        if (
          ...
          console.log(
            `    Found new smallest eligible: ${(value / 100000000).toFixed(3)} VRSC (${value} satoshis) for ${address}`
          );
        }
    ...
    return {
      ...
      eligibleValue,
      largest,
      smallestEligible: smallestEligible, // Keep null if no eligible UTXOs
    };
  } catch (error) {
...
async function updateUTXOStats(address, utxoData) {
  try {
    console.log(
      `    Updating database: smallestEligible = ${utxoData.smallestEligible} (${utxoData.smallestEligible ? (utxoData.smallestEligible / 100000000).toFixed(3) : 'null'} VRSC)`
    );
...
async function processVerusID(verusID, currentHeight, index, total) {
  try {
    ...
    process.stdout.write(
      `\r[${progress}%] Processing ${verusID.friendly_name || verusID.address}...`
    );
    ...
    if (utxoData.total > 0) {
      console.log(
        `\r✓ ${verusID.friendly_name || verusID.address}: ${utxoData.total} UTXOs (${utxoData.eligible} eligible, ${utxoData.cooldown} cooldown) = ${(utxoData.totalValue / 100000000).toFixed(2)} VRSC`
      );
    }
  } catch (error) {
    console.log(
      `\r❌ ${verusID.friendly_name || verusID.address}: ${error.message}`
    );
    stats.errors++;
  } finally {
...
async function main() {
  try {
    ...
    console.log(
      `   Eligible for staking: ${stats.totalEligible.toLocaleString()}`
    );
    console.log(`   Time taken: ${totalTime.toFixed(1)} seconds`);
    console.log(
      `   Rate: ${(stats.totalProcessed / totalTime).toFixed(1)} addresses/sec\n`
    );
