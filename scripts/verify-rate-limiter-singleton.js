...
async function verifyRateLimiter() {
  try {
    ...
    console.log('─────────────────────────────────────────────────────');
    console.log(
      `Per Second:  ${stats.current.perSecond}/${stats.limits.perSecond} requests`
    );
    console.log(
      `Per Minute:  ${stats.current.perMinute}/${stats.limits.perMinute} requests`
    );
    console.log(
      `Per Hour:    ${stats.current.perHour}/${stats.limits.perHour} requests`
    );
    ...
    console.log(
      '✅ HTTP Rate Limiter: lib/utils/validation.ts (separate, for API endpoints)'
    );
